...
export default function Footer() {
  ...
  const rawSocials = [
    { name: "Instagram", url: settings.instagram_url, icon: InstagramIcon, hoverColor: "hover:text-pink-600 hover:border-pink-500" },
    { name: "WhatsApp", url: settings.whatsapp_url, icon: WhatsappIcon, hoverColor: "hover:text-emerald-600 hover:border-emerald-500" },
    { name: "Facebook", url: settings.facebook_url, icon: FacebookIcon, hoverColor: "hover:text-blue-600 hover:border-blue-500" },
  ];

  const configuredSocials = rawSocials.filter(s => s.url && s.url.trim() !== "");

  // Fallback defaults if none configured yet
  const activeSocials = configuredSocials.length > 0 ? configuredSocials : [
    { name: "Instagram", url: "https://instagram.com", icon: InstagramIcon, hoverColor: "hover:text-pink-600 hover:border-pink-500" },
    { name: "WhatsApp", url: "https://wa.me/919123456789", icon: WhatsappIcon, hoverColor: "hover:text-emerald-600 hover:border-emerald-500" },
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2.5">
              {activeSocials.map((item, i) => {
                ...
                if (targetUrl && !targetUrl.startsWith("http://") && !targetUrl.startsWith("https://") && !targetUrl.startsWith("wa.me")) {
                ...
                return (
                  <a
                    ...
                    className={`flex h-9 w-9 items-center justify-center rounded-lg border border-border text-muted transition-all duration-200 ${item.hoverColor} hover:scale-105`}
