        ...
        <nav class="flex h-16 items-center justify-between gap-4" aria-label="Global">
            ...
            <div class="flex items-center gap-3 shrink-0">
                <a href="/" class="flex items-center gap-2.5 group">
                    <img src="{{ asset('logo2.png') }}" alt="SABHA"
                        class="h-11 w-11 object-contain transition-all duration-300 group-hover:scale-105" />
                    <div class="flex flex-col">
                        <div class="flex items-center gap-1.5">
                            <span class="text-xl font-black tracking-tight text-primary-dark leading-none">SABHA</span>
                        </div>
                        <span
                            class="text-[10px] font-bold text-muted uppercase tracking-wider leading-tight mt-0.5 hidden sm:block">Business
                            Network</span>
                    </div>
                </a>
            </div>
            ...
            <div class="hidden lg:flex lg:items-center lg:gap-1 xl:gap-1.5">
                ...
                <div class="relative" x-on:click.outside="eventsDropdownOpen = false">
                    <button type="button"
                        ...
                        <x-icon name="chevron-down" class="h-3.5 w-3.5 transition-transform duration-200"
                            x-bind:class="eventsDropdownOpen ? 'rotate-180 text-primary' : 'text-slate-400'" />
                    </button>

                    {{-- Events Dropdown Menu --}}
                    <div x-show="eventsDropdownOpen" x-cloak x-transition:enter="transition ease-out duration-150"
                        ...
                        <a href="/events" x-on:click="eventsDropdownOpen = false"
                        ...
                        <a href="/events?filter=current" x-on:click="eventsDropdownOpen = false"
                            ...
                            <div>
                                <span
                                    class="block leading-tight text-emerald-800">{{ __('site.nav.booking_available') }}</span>
                                <!-- <span class="text-[10px] font-normal text-slate-400">{{ __('site.nav.booking_available_desc') }}</span> -->
                            </div>
                        </a>

                        <a href="/events?filter=upcoming" x-on:click="eventsDropdownOpen = false"
                            ...
                            <div>
                                <span
                                    class="block leading-tight text-amber-800">{{ __('site.nav.booking_soon') }}</span>
                                <!-- <span class="text-[10px] font-normal text-slate-400">{{ __('site.nav.booking_soon_desc') }}</span> -->
                            </div>
                        </a>

                        <a href="/events?filter=past" x-on:click="eventsDropdownOpen = false"
                            ...
                            <div
                                class="flex h-6 w-6 items-center justify-center rounded-lg bg-slate-100 text-slate-600">
                ...
                <div class="relative" x-on:click.outside="aboutDropdownOpen = false">
                    ...
                    {{-- About Dropdown Menu --}}
                    <div x-show="aboutDropdownOpen" x-cloak x-transition:enter="transition ease-out duration-150"
                        x-transition:enter-start="opacity-0 translate-y-1 scale-95"
                        x-transition:enter-end="opacity-100 translate-y-0 scale-100"
                        x-transition:leave="transition ease-in duration-100"
                        x-transition:leave-start="opacity-100 translate-y-0 scale-100"
                        ...
                        <a href="/about" x-on:click="aboutDropdownOpen = false"
                        ...
                        <a href="/trustees" x-on:click="aboutDropdownOpen = false"
                            ...
                            <div
                                class="flex h-6 w-6 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600">
                                <x-icon name="users" class="h-3.5 w-3.5" />
                            </div>
                            <span>{{ __('site.nav.trustees') }}</span>
                        </a>

                        <a href="/contact" x-on:click="aboutDropdownOpen = false"
            ...
            <div class="hidden lg:flex lg:items-center lg:gap-2.5 xl:gap-3 shrink-0">
                @auth
                    ...
                    {{-- Chat Button with Icon (Available for all users) --}}
                    <a href="/chat"
                    ...
                    <div class="relative" x-on:click.outside="userDropdownOpen = false">
                        <button type="button" x-on:click="userDropdownOpen = !userDropdownOpen; aboutDropdownOpen = false"
                            ...
                            @if (media_url($user->avatar))
                                <img src="{{ media_url($user->avatar) }}" alt="{{ $user->name }}"
                                    class="h-7 w-7 rounded-lg object-contain bg-slate-100 shrink-0 shadow-2xs border border-slate-200/80" />
                            @else
                                <div
                                    class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-[#00379D] to-[#0F3459] text-white text-[11px] font-black uppercase shadow-xs">
                                    {{ strtoupper(substr($user->name, 0, 1)) }}
                                </div>
                            @endif
                            <div class="flex flex-col text-left max-w-[120px]">
                                <span
                                    class="truncate text-xs font-bold text-slate-900 leading-none">{{ $user->name }}</span>
                                <span
                                    class="text-[10px] font-medium text-slate-500 uppercase tracking-wider leading-tight mt-0.5">{{ $user->role === 'admin' ? __('site.nav.admin_role') : ($user->role === 'sub_admin' ? __('site.nav.sub_admin_role') : __('site.nav.member_role')) }}</span>
                            </div>
                        ...
                        <div x-show="userDropdownOpen" x-cloak x-transition:enter="transition ease-out duration-150"
                            ...
                            @if ($user->canAccessAdminArea())
                                <a href="/admin" x-on:click="userDropdownOpen = false"
                ...
                <div
                    class="flex items-center gap-0.5 bg-slate-200/70 p-0.5 rounded-xl border border-slate-300/50 text-xs font-bold shadow-inner">
            ...
            <div class="flex items-center gap-2 lg:hidden">
                ...
                <div class="flex items-center gap-0.5 bg-slate-200/70 p-0.5 rounded-lg text-xs font-bold">
                    <a href="{{ route('lang.switch', 'en') }}"
                        class="px-2 py-0.5 rounded {{ $locale === 'en' ? 'bg-primary text-white' : 'text-slate-600' }}">EN</a>
                    <a href="{{ route('lang.switch', 'gu') }}"
                        class="px-2 py-0.5 rounded {{ $locale === 'gu' ? 'bg-primary text-white' : 'text-slate-600' }}">GU</a>
                </div>
                ...
                <button type="button"
                    ...
                    x-on:click="mobileMenuOpen = true" aria-label="Open menu">
