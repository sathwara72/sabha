            ...
            <aside class="lg:col-span-4 glass-card p-0 overflow-hidden flex flex-col h-full {{ $activeId ? 'hidden lg:flex' : 'flex' }}">
                <div class="p-3 border-b border-border space-y-2">
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <a href="{{ route('chat.groups.create') }}" class="inline-flex items-center gap-1 rounded-lg bg-primary-soft px-2.5 py-1.5 text-[11px] font-bold text-primary hover:opacity-90 transition-all cursor-pointer">
                            <x-icon name="users" class="h-3 w-3" /> New Group
                        </a>
                    </div>
                    <div class="relative">
                ...
                <div class="flex-1 overflow-y-auto">
                    @if (trim($search) !== '')
                        <div class="p-2 space-y-3">
                            <div class="space-y-1">
                                <p class="px-2 py-1 text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Members</p>
                                ...
                                @empty
                                    <p class="px-2 py-3 text-xs text-muted-foreground italic text-center">No members found.</p>
                                @endforelse
                            </div>

                            <div class="space-y-1 border-t border-border/60 pt-2">
                                <p class="px-2 py-1 text-[11px] font-bold text-muted-foreground uppercase tracking-wide">Groups to join</p>
                                @forelse ($groupResults as $result)
                                    <div class="w-full flex items-center gap-2.5 p-2 rounded-xl hover:bg-surface transition-colors">
                                        <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary-soft text-primary font-bold text-xs overflow-hidden">
                                            @if (media_url($result->avatar))
                                                <img src="{{ media_url($result->avatar) }}" alt="{{ $result->title }}" class="h-full w-full object-cover" />
                                            @else
                                                <x-icon name="users" class="h-4 w-4" />
                                            @endif
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <p class="text-xs font-bold text-foreground truncate">{{ $result->title }}</p>
                                        </div>
                                        <button
                                            type="button"
                                            wire:click="joinGroup({{ $result->id }})"
                                        ...
                                        >
                                            {{ $result->join_setting === 'approval_required' ? 'Request' : 'Join' }}
                                        </button>
                                    </div>
                                @empty
                                    <p class="px-2 py-3 text-xs text-muted-foreground italic text-center">No groups found.</p>
                                @endforelse
                            </div>
                        </div>
                    @else
                        @if ($conversations->isEmpty())
                            <div class="p-6 text-center text-xs text-muted-foreground italic">
                                No conversations yet. Search a member or group above, or start a new group.
                            </div>
                        @else
                            <div class="divide-y divide-border/60">
                                @foreach ($conversations as $conv)
                                    ...
                                    >
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-center justify-between gap-2">
                                                <p class="text-xs font-bold text-foreground truncate">{{ $conv['title'] ?? 'Unknown' }}</p>
                                            ...
                                            <div class="flex items-center justify-between gap-2 mt-0.5">
                                                <p class="text-[11px] text-muted-foreground truncate">
                                                    @if ($conv['last_message'])
                                                        ...
                                                        @else
                                                            {{ $conv['type'] === 'group' ? $conv['last_message']->sender->name . ': ' : '' }}{{ Str::limit(strip_tags($conv['last_message']->body), 40) }}
                                                        @endif
                                                    @else
                                                        Say hello!
                                                    @endif
