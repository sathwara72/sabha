...
>
    ...
    <div x-show="$store.auth.loginOpen" x-transition class="relative w-full max-w-sm rounded-2xl border border-border bg-white p-5 shadow-xl">
        ...
        <div class="mb-4 text-center">
            <img src="{{ asset('logo.png') }}" alt="SABHA" class="mx-auto h-9 w-9 rounded-full object-contain mb-2.5" />
        ...
        <form x-on:submit.prevent="submit" class="space-y-3">
            <div class="space-y-1">
                ...
                <div class="group relative">
                    ...
                    <input type="email" required x-model="email" placeholder="{{ __('site.auth.email_placeholder') }}" class="w-full rounded-xl border border-border bg-white py-2 pl-9 pr-3 text-xs text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:border-primary" />
                </div>
            </div>
            ...
            <div class="space-y-1">
                ...
                <div class="group relative">
                    ...
                    <input :type="showPassword ? 'text' : 'password'" required x-model="password" placeholder="{{ __('site.auth.password_placeholder') }}" class="w-full rounded-xl border border-border bg-white py-2 pl-9 pr-9 text-xs text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:border-primary" />
                    <button type="button" x-on:click="showPassword = !showPassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors p-0.5" tabindex="-1">
