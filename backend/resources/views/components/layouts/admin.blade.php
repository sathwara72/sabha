...
<body class="min-h-screen bg-background font-outfit text-foreground" x-data="{ sidebarOpen: false }" x-init="$watch('sidebarOpen', (v) => document.body.style.overflow = v ? 'hidden' : '')">
    <div class="min-h-screen bg-background font-outfit text-foreground flex">
        ...
        @php
            $menuItems = [
                ['name' => 'Overview', 'icon' => 'layout-dashboard', 'href' => route('admin.dashboard'), 'active' => request()->routeIs('admin.dashboard')],
                ['name' => 'Businesses', 'icon' => 'briefcase', 'href' => route('admin.businesses.index'), 'active' => request()->routeIs('admin.businesses.*')],
                ['name' => 'Events', 'icon' => 'calendar', 'href' => route('admin.events.index'), 'active' => request()->routeIs('admin.events.*')],
                ['name' => 'Bookings', 'icon' => 'shield-check', 'href' => route('admin.bookings.index'), 'active' => request()->routeIs('admin.bookings.*')],
                ['name' => 'Users', 'icon' => 'users', 'href' => route('admin.users.index'), 'active' => request()->routeIs('admin.users.*')],
        ...
        <aside class="hidden lg:flex w-64 border-r border-border bg-white p-5 flex-col fixed h-full z-50">
            @include('components.layouts._admin-sidebar-content', ['menuItems' => $menuItems])
        </aside>
